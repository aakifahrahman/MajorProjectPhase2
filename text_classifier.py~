...
#To obtain book ID from the user
asin = input('Enter BookID : ')
...
#for line in data:
#    if line['asin'] == asin:
#        data_asin.append(line)

# Extract relevant fields in dataset -- category label and text content
data_pared = data.map(lambda line: (if line['asin'] == asin: (line['overall'], line['reviewText'])))
